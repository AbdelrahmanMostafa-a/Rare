...
if "result" in st.session_state:
    ...
    # ----------------------------------------------------------
    # نسبة العملاء المتجاوزين - من عملاء PDF فقط (أهم مؤشر في الداشبورد)
    # ----------------------------------------------------------
    overdue_ratio = compute_overdue_ratio(result, followup=st.session_state.get("followup"))

    if overdue_ratio is not None:
        st.subheader("🎯 نسبة العملاء المتجاوزين (من عملاء PDF فقط)")
        ...
        rc3.metric("النسبة", f"{pct:.1f}%")

        msg = (
            f"{overdue_ratio['overdue_customers']} من {overdue_ratio['pdf_customers']} عميل "
            f"({pct:.1f}%) من عملاء PDF عندهم تجاوز في المدة المطلوب سدادها"
        )
        if pct >= 50:
