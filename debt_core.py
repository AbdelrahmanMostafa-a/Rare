...
def compute_overdue_ratio(result, followup=None):
    """
    نسبة العملاء المتجاوزين للمدة المطلوب سدادها من إجمالي العملاء
    الموجودين في PDF (وبس - عملاء المديونية اللي مش في PDF مالهمش دعوة
    بالحساب ده). ترجع None لو عمود التجاوز غير متاح في ملف المديونية.
    """
    if OVERDUE_COL not in result.columns:
        return None
    ...
    pdf_customers = int(result["رصيد PDF"].notna().sum())
